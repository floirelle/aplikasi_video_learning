    ...
    <div class="container">
        <div class="row" style="padding: 2%">
            <div class="login-form bg-light p-4">
                <form class="row g-3" action="/insert-video" method="POST">
                    @csrf
                    <input type="hidden" name="session_id" value="{{ $session->session_id }}">
                    <input type="hidden" id="video_link" name="video_link">
                    <input type="hidden" id="folder" value="{{ $session->course->course_code }}">
                    <h4 style="color: #285185">Insert New Video {{ $session->course->course_code }} -
                        {{ $session->course->course_name }} - {{ $session->session_name }}</h4>
                    <div class="col-12">
                        @if ($errors->any())
                        ...
                        <div class="form-floating mb-3">
                            <textarea class="form-control" placeholder="Video Software Description"
                                ...
                                style="height: 150px">{{ old('video_software_description') }}</textarea>
                        ...
                        {{-- Upload Video --}}
                        <button class="btn btn-warning float-end" id="send" disabled type="submit">Add</button>
                    </div>
                </form>
            </div>
        </div>
